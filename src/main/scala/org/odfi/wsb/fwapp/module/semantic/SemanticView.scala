...
trait SemanticView extends LibraryView with FWAppFrameworkView with SemanticUIImplView with AssetsGeneratorView with WebsocketView with FWAppTempBufferView {
    ...
    def semanticTooltipTopLeft(text: String) = {
        data("tooltip", text)
        data("position", "top left")
    }
    def semanticTooltipTopCenter(text: String) = {
        data("tooltip", text)
        data("position", "top center")
    }
    def semanticTooltipTopRight(text: String) = {
        data("tooltip", text)
        data("position", "top right")
    }
    def semanticTooltipBottomLeft(text: String) = {
        data("tooltip", text)
        data("position", "bottom left")
    }
    def semanticTooltipBottomCenter(text: String) = {
        data("tooltip", text)
        data("position", "bottom center")
    }
    def semanticTooltipBottomRight(text: String) = {
        data("tooltip", text)
        data("position", "bottom right")
    }
    def semanticTooltipRightCenter(text: String) = {
        data("tooltip", text)
        data("position", "right center")
    }
    def semanticTooltipLeftCenter(text: String) = {
        data("tooltip", text)
    ...
    /**
     * Creates div for fluid input and add content
      * No input is created
     */
...
;*/
    ...
    /**
     * Call on current node to init as modal
     */
    def semanticInitModal = {

        this.jqueryGenerateOnLoad("semantic-modal-init" + currentNode.getId) match {
            case Some(generator) =>

                generator.println("console.info('Init Modal');")

                generator.println(s"""$$("#${currentNode.getId}").modal({
                  blurring:true
              });""")
    ...
    def semanticModalShowOnClick(modal: Div[HTMLElement, Any]) = {
        +@("onclick", s"""$$('#${modal.getId}').modal('show')""")
    ...
    def semanticSetProgress(pid: String, percent: Int, message: String = "") = {
        ...
        pmessage.Percent = percent.toDouble
        message match {
            case "" =>
            case m =>
                pmessage.Message = m
